...
st.markdown(
    ...
    <style>
      ...
      .tool-title {font-size: 1.35rem; font-weight: 700; margin-bottom: 0.2rem;}
      .tool-sub {color: #666; font-size: 0.92rem; margin-bottom: 0.8rem;}
      .divider-vert {
...
left, middle, right = st.columns([1, 0.03, 1])
...
with left:
    ...
    st.markdown('<div class="tool-sub">Paste text and process ticker tags directly on this page.</div>', unsafe_allow_html=True)

    st.text_area('Input text', key='ticker_input_text', height=300, label_visibility='visible')
...
with middle:
    st.markdown('<div class="divider-vert"></div>', unsafe_allow_html=True)

with right:
    st.markdown('<div class="tool-panel">', unsafe_allow_html=True)
    st.markdown('<div class="tool-title">Stock Graph Generator</div>', unsafe_allow_html=True)
    st.markdown('<div class="tool-sub">Generates the same PNG chart logic as the original desktop version.</div>', unsafe_allow_html=True)
